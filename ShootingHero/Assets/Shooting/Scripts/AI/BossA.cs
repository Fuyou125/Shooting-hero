...
namespace Shooting.Gameplay
{
    public class BossA : EnemyBase
    {
        ...
        void Start()
        {
            m_DamageControl = GetComponent<DamageController>();  // 获取 DamageControl 组件
        ...
        {
            EnemyMove movement = GetComponent<EnemyMove>();  // 获取敌人移动组件
        ...
        public void ShootBullet()
        {
            ...
            obj.GetComponent<ProjectileMove>().m_TurnSpeed = 00;
        ...
        IEnumerator CoroutineHandleDeath()
        {
            ...
            Destroy(obj1, 6);
            CameraController.m_Current.StartShake(.6f, .2f);  // 相机震动
            DropItem(20);  // 掉落物品
        ...
        public override void DropItem(int count)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                Vector3 v = Helpers.RotatedLength(i * 18, 10) + new Vector3(0, 15, 0);  // 随机位置
            ...
            for (int i = 0; i < 20; i++)
            {
                GameObject obj1 = Instantiate(m_ItemPrefabs[0]);
                obj1.transform.position = transform.position;
                Vector3 v = Helpers.RotatedLength(i * 18, 15) + new Vector3(0, 20, 0);  // 随机位置
                obj1.GetComponent<Rigidbody>().velocity = v;  // 设置物理速度
            }
        }
    }
