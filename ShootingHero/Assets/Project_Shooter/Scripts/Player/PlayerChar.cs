...
namespace Shooter.Gameplay
{
    ...
    {
        [HideInInspector]
        public DamageControl m_DamageControl; // 伤害控制组件
        public static PlayerChar m_Current; // 当前玩家实例
        ...
        //public WeaponControl m_MyWeaponControl; // 武器控制（暂时没有使用）
        ...
        public Weapon_Base[] m_Weapons; // 玩家所拥有的武器
        [HideInInspector]
        public int m_WeaponNum = 0; // 当前装备的武器编号

        public TargetObject m_TempTarget; // 临时瞄准目标
        ...
        [HideInInspector]
        public PlayerPowers m_PlayerPowers; // 玩家技能控制
        ...
        void Start()
        {
            ...
            m_ShieldObject.transform.SetParent(null); // 将护盾对象从父物体移除
            m_WeaponPowerParticle.SetActive(false); // 关闭武器能量粒子效果
        }
        ...
        void Update()
        {
            ...
            if (m_InControl)
            {
                m_Input_Fire = PlayerControl.MainPlayerController.Input_FireHold; // 获取开火输入

                // 判断是否使用技能（如投掷手雷）
                if (Input.GetKeyDown(KeyCode.Z))
                {
                    if (m_PlayerPowers.m_HavePower) // 判断是否有技能
                    {
                        switch (m_PlayerPowers.m_PowerNum)
                        {
                            case 0: // 当前技能：投掷手雷
                                ...
                                {
                                    ThrowGrenade(); // 投掷手雷
                                    m_PlayerPowers.m_AmmoCount--; // 消耗一个手雷
                                }
                                break;
                        }
                    }
                ...
                // 查找目标
                List<TargetObject> targets = TargetsControl.m_Main.m_Targets; // 获取目标列表
                ...
                if (m_MovementInput != Vector3.zero)
                {
                    ...
                    m_TurnBase.rotation = Quaternion.Lerp(m_TurnBase.rotation, Quaternion.LookRotation(faceDirection), 10 * Time.deltaTime); // 平滑过渡旋转
                }
            ...
            {
                m_WpnPowerTime -= Time.deltaTime; // 每帧减少能量时间
                if (m_WpnPowerTime <= 0)
                {
        ...
        {
            CameraControl.m_Current.StartShake(.2f, .1f); // 震动效果
        }
        ...
        public void ThrowGrenade()
        {
            ...
            PlayerGrenade g = obj.GetComponent<PlayerGrenade>(); // 获取手雷控制
        ...
        public void HandlePickup(string itemType, int count)
        {
            if (itemType == "Gem")
            {
                PlayerControl.MainPlayerController.m_GemCount++; // 增加宝石数量
            }
            ...
            {
                m_PlayerPowers.SetNewPower(0); // 设置新的技能
            }
