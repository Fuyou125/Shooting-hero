...
namespace Shooter.Gameplay
{
    public class Enemy : MonoBehaviour
    {
        ...
        {
            m_DamageControl = GetComponent<DamageControl>();  // 重新获取伤害控制组件
        ...
        public virtual void DropItem(int count)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                obj1.GetComponent<Rigidbody>().velocity = new Vector3(Random.Range(-5, 5), Random.Range(10, 20), Random.Range(-5, 5));  // 随机速度
                obj1.GetComponent<Rigidbody>().angularVelocity = new Vector3(Random.Range(-20, 20), Random.Range(-20, 20), Random.Range(-20, 20));  // 随机角速度
            }
        }
