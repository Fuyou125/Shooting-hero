...
namespace Shooting.Gameplay
{
    public class Pickable : MonoBehaviour
    {
        ...
        [HideInInspector]
        public PlayerCharacter m_TouchedPlayer;  // 被触碰的玩家角色
        ...
        void Start()
        {
            ...
            Invoke("AllowPick", 0.5f);  // 延迟0.5秒后允许拾取
        }
        ...
        {   
            ...
            if (!picked && m_CanPick)
            {
                ...
                if (Vector3.Distance(transform.position, PlayerCharacter.m_Current.transform.position) <= 5f)
                {
                    m_TouchedPlayer = PlayerCharacter.m_Current;  // 记录当前接触的玩家
